...
class UserWayReportController extends BaseReportController {
     ...
     */
    public function userWayReport() {
        $oRows = Transaction::selectRaw('DATE_FORMAT(created_at,\'%Y-%m-%d\') as `date`,user_id,count(0) as nums,sum(amount) as amount ,type_id,way_id,merchant_id')->whereIN('type_id', [ 7, 8, 11, 12 ])->where('way_id', '>', 0)->groupBy([ 'type_id', 'date', 'way_id' ]);
        ...
        $aRows = $oRows->get()->toArray();

        $aData = $this->setUserWayReportData($aRows);
     ...
     * 详情页
     */
    public function showUserWayReport($date, $user_id, $way_id) {
        $oRows = Transaction::selectRaw('DATE_FORMAT(created_at,\'%Y-%m-%d\') as `date`,user_id,count(0) as nums,sum(amount) as amount ,type_id,way_id,merchant_id')->groupBy([ 'type_id', 'user_id', 'way_id' ])->whereDate('created_at', $date)->where('way_id', $way_id)->where('user_id', $user_id);
        $oRows = $this->publicCondition($oRows);
        $oRows = $oRows->get()->toArray();
        $aData = $this->setUserWayReportData($oRows);
        $data = reset($aData);
    ...
    public function searchConditions($oRows, &$aSearchData) {
        ...
        if($username){
            $oRows->with('user')->whereHas('user', function($q) use ($username) {
