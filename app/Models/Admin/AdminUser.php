...
class AdminUser extends BaseModel implements AuthenticatableContract,
    ...
    protected $fillable = ['username', 'name', 'email', 'password', 'password_confirmation', 'language', 'merchant_id','menu_link', 'menu_context', 'actived', 'signin_at'];
    protected $hidden = ['remember_token'];

    public static $titleColumn = 'username';
    public static $enabledBatchAction = true;
    public static $columnForList = ['id', 'username', 'name', 'language', 'actived'];
    ...
    public function roles() {
        return $this->belongsToMany('App\Models\AdminRole', 'admin_role_admin_user', 'user_id', 'role_id')->withPivot(AdminRoleUser::$columnsForPivot)->withTimestamps();
    }
    ...
    public function beforeDelete() {
        ...
        if (AdminRoleUser::checkUserRoleRelation(AdminRole::ADMIN, $this->id)) {
            return false;
        }
        return true;
    }
    ...
    public function resetPassword($aFormData, $bClearSetTime = true) {
        ...
        !$bClearSetTime or $this->pwd_seted_at = null;
        if ($aReturnMsg['success']) {
            $this->password = $aReturnMsg['msg'];
            if ($bSucc = $this->save()) {
                $aReturnMsg['msg'] = ___('_user.password-updated');
            }
        }
        return $aReturnMsg;
    }
...
//    }
    ...
    public function setPwdChangedTime($sDateTime = null) {
        !empty($sDateTime) or $sDateTime   = date('Y-m-d H:i:s');
        $aConditions = [
            'id' => ['=', $this->id]
        ];
        $data        = [
            'pwd_seted_at' => $sDateTime
        ];
