...
class BaseModel extends Ardent
{
     ...
     * @var bool
     */
    public static $merchantColumn = false;

    function __construct(array $attributes = [])
    {
        parent::__construct($attributes);
        $this->comaileLangPack();
        $this->setCustormForList();
    }
    ...
    public static function find($id, $columns = ['*']) {
        if (static::$cacheLevel == self::CACHE_LEVEL_NONE) {
            if(static::$merchantColumn && $iMerchatId = session('merchant_id')) {
                return static::where(static::$merchantColumn, $iMerchatId)->find($id, $columns);
            }else{
        ...
        $aAttributes = Cache::get($key);
        if(static::$merchantColumn && $iMerchatId = session('merchant_id')) {
            if($aAttributes[static::$merchantColumn] != $iMerchatId){
                $aAttributes = [];
            }
        }
        if ($aAttributes) {
     ...
     * @param  string $data [缓存key]
     * @return string       [加前缀的缓存key]
     */
    ...
    public static function doWhere($aConditions = []) {
        ...
        $oQuery = static::getCustormModel($oQuery);
        foreach ($aConditions as $sColumn => $aCondition) {
            switch ($aCondition[0]) {
                case '=':
                    $oQuery = is_null($aCondition[1]) ? $oQuery->whereNull($sColumn) : $oQuery->where($sColumn, '=', $aCondition[1]);
    ...
    public static function & getTitleList($bOrderByTitle = true) {
        ...
        $oModels = static::getCustormModel();
    ...
    public function & getValidationErrorString() {
        ...
        foreach ($this->validationErrors->toArray() as $sColumn => $sMsg) {
            $aErrMsg[] = ___(static::$defaultLangPack . '.' . strtolower($sColumn)) . ': ' . implode(',', $sMsg);
        }
    ...
    public static function translate($sText, $iUcType = 3, $aReplace = []) {
        return ___(static::$defaultLangPack . '.' . strtolower($sText), $aReplace, $iUcType);
    }

    public static function translateArray(& $aTexts, $iUcType = 2, $aReplace = []) {
        static::comaileLangPack();
        foreach ($aTexts as $key => $sText) {
            $aTexts[$key] = ___(static::$defaultLangPack . '.' . strtolower($sText), $aReplace, $iUcType);
        }
    }
     ...
     */
     public function downloadExcel($aTitles, $aData, $sFileName, $sDir='./',$sOutPutType = 'Command') {
         if (count($aData) <= 0) {
             return false;
         }
     ...
     * 一般用于 select 搜索条件数据
     *
     * @param array $aColumns 字段数组['key1','key2']
     * @param array $aWheres  二维查询条件数组[[id=>x],[type=>y]],或一维数组查询条件[id=>x]
     * @param bool  $isOrderByValue 是否按照值排序，默认按照键排序
     *
     * @return array 键为key1,值为key2,[key1=>key2]
     */
    public static function getSelectSearchArrays($aColumns = [ 'id', 'name' ], $aWheres = [], $isOrderByValue = false) {
        if(is_array($aColumns) && count($aColumns) < 2) return [];
        $model = static::getCustormModel();
        if(!empty($aWheres)){
            static::addWheres($model,$aWheres);
        }

        $model = $model->orderBy($aColumns[$isOrderByValue ? 1 : 0],'asc');
        return $model->get($aColumns)->mapWithKeys(function($item) use ($aColumns) {
            return [ $item[ $aColumns[0] ] => $item[ $aColumns[1] ] ];
        });
    }
    ...
    /**
     * 获取模型与关系数据特定数组
     * @param array|string $aColumns   字段:默认[id,name] 'sRelationColumn',如需修改name,传数组['Key','Value','RelationColumn']
     * @param  array     $aWheres    条件数组
     * @param  array    $aRelations 关系数据 必须为[$id=>$name|$title] 的数组
     * @param bool  $isOrderByValue 是否按照值排序，默认按照键排序
     *
     * @return array            collect对象 [[$Key=>$aRelations->name -- $Value],[]....]
     */
    public static function getModelRelationArrays($aColumn ,$aRelations, $aWheres=[], $isOrderByValue = false) {
        if(is_array($aColumn)) $aColumns = $aColumn;
        else $aColumns = ['id','name',$aColumn];
        $model = static::getCustormModel();
        if(!empty($aWheres)){
            static::addWheres($model,$aWheres);  //dd($model,$aWheres);
        }

        $model = $model->orderBy($aColumns[2],'asc');
        $model = $model->orderBy($aColumns[$isOrderByValue ? 1 : 0],'asc');

        return $model->get($aColumns)->mapWithKeys(function($item) use ($aColumns, $aRelations) {
            return [ $item[ $aColumns[0] ] => $aRelations[ $item[ $aColumns[2] ] ].'-'.$item[ $aColumns[1] ] ];
        });
     ...
     *
     * @param $model
     * @param $aWheres $aWheres=[[where],[where]...]  |  [where]
     *                 {where=['column'=>'param'] | ['column','in|=|>|<|between..','params']}
     */
    public static function addWheres(&$model,$aWheres){
        foreach($aWheres as $aWhere){
            if(count($aWhere)==3){
                $model = static::setModelCondition($model,$aWhere);
            }else{
                if(!is_array($aWhere)){
                    if(count($aWheres)==3){
                        $model = static::setModelCondition($model,$aWheres);break;
                    }else{
                        $model=$model->where($aWheres);break;
                    }
                }
                $model=$model->where($aWhere);
            }
        }
    }
     ...
     * 拼接模型条件
     * @param $model
     * @param $aWhere $aWhere=['column','(= > < in between ...)','param']
     *
     * @return mixed
     */
    public static function setModelCondition($model,$aWhere){
        switch($aWhere[1]){
            case 'in':
                return $model->whereIn($aWhere[0],$aWhere[2]);
            case 'between':
                return $model->whereBetween($aWhere[0],$aWhere[2]);
            default:
                return $model->where($aWhere[0],$aWhere[1],$aWhere[2]);
        }
    }

    protected function deleteThirdCache($aKeys=[]){
        if(empty($aKeys)) return false;
        if(!is_array($aKeys)) $aKeys = [$aKeys];
    ...
    protected static function getCustormModel($oModel = null){
        ...
        if(static::$merchantColumn && $iMerchantId = session('merchant_id')) {
            $oModel = $oModel->where(static::$merchantColumn,$iMerchantId);
        }
        return $oModel;
    }
     ...
     * 设置列表显示，有商户的显示商户
     */
    protected function setCustormForList(){
        if(static::$merchantColumn && !in_array(static::$merchantColumn,static::$columnForList)){
            array_splice(static::$columnForList,(int)in_array('id',static::$columnForList),0,[static::$merchantColumn]);
        }
        if(static::$merchantColumn){
            static::$htmlSelectColumns[static::$merchantColumn] = 'aMerchantIds';
        }
