...
class District extends BaseModel {
    ...
    {
        $aProvinces = static::whereNull('parent_id')->where('disabled', '=', self::ENABLED)->get(['id', 'province_id', 'parent_id', 'name']);
        $data = [];
    ...
    public static function getCities($iProvinceId){
        $oCities = static::where('parent_id','=',$iProvinceId)->where('disabled', '=', self::ENABLED)->get(['id', 'name']);
        $data = [];
        foreach ($oCities as $oCity) {
            $data[$oCity->id] = $oCity->name;
        }
        return $data;
    }
    ...
    {
        $aColumns = ['id', 'name'];
        $oQuery = $parent_id ? static::where('parent_id', '=', $parent_id) : static::whereNull('parent_id');
        ...
        foreach ($aProvinces as $oProvince) {
            ...
            $aCities = static::where('parent_id', '=', $oProvince->id)->orderBy('sequence', 'asc')->get($aColumns);
    ...
    public static function & getDistincts(){
        $oProvinces = static::getAllProvinces();
        $data = [];
        foreach ($oProvinces as $iProvince => $sProvince) {
            $data[$iProvince] = $sProvince;
            $aCities = static::getCities($iProvince);
            foreach ($aCities as $iCityId => $sCity) {
                $data[$iCityId] = '--' . $sCity;
            }
        }
        return $data;
