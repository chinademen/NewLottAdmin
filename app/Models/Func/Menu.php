...
//    ];
    ...
    private static function & _getAvailableMenus($aRights = [], $aRealm = [Functionality::REALM_ADMIN]) {
        ...
        $aMenus = [];
        $sDefaultNamespace = Config::get('namespace-map.ControllerDefaultNamespace'); // static::$defaultNamespace;
        // pr($aRealm);
        // pr(implode(',', $aRights));
    ...
    public static function getMainMenu() {
        return static::whereNull('parent_id')->orderBy('sequence')->pluck('title', 'id');
    }

    protected function getTitleTextAttribute() {
        return $this->attributes['title'] .' ('.___( '_function.' . $this->attributes['title']).')';
    }
