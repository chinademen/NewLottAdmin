...
//        'formated_default_value' => 'between:1,1024',
    ...
    public static function get($sItem) {
        return static::readValue($sItem);
    }
    ...
    protected static function _readByItem($sItem) {
        ...
        if (static::$cacheLevel != self::CACHE_LEVEL_NONE) {
            ...
            $key = static::createCacheKey($sItem);
            if (!$data = Cache::get($key)) {
                $obj = static::where('item', '=', $sItem)->get()->first();
                if (empty($obj)) {
                    die('missing set:' . $sItem);
                }
