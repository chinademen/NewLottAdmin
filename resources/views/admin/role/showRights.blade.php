...
<?php
$sSelectedAllText = ___('_basic.select all') . ' / ' . ___('_basic.cancel all');
?>
...
<div class="col-md-12">
    ...
    <div class="h2">{{ ___('Bind Rights For Role ' . $sRoleName) }} ASDFSDFSD
...
@section('end')
    ...
    <?php
        ...
        foreach ($datas as $key => $value) {
            $item = ['id' => $value->id,'level' => $value->level,'parent_id' => $value->parent_id,'forefather_ids' => $value->forefather_ids,'title' => $value->title,'controller' => $value->controller,'action' => $value->action,'desc' => ___('_function.' . ($value->title ? $value->title : $value->controller . ' ' . $value->action),[],3)];
    if ($readonly) {
