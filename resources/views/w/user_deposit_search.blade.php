            ...
            <table>
                <tr>
                    ...
                    <td  style="width:160px">
                        <input class="form-control boot-time input-xs" type="text" name="put_at[]" value="{{@$aSearchFields['put_at'][0]}}" >
                    </td>
                    <td style="width:80px" class="text-right">至：</td>
                    <td  style="width:180px">
                            <input class="form-control boot-time input-xs"  type="text" name="put_at[]" value="{{@$aSearchFields['put_at'][1]}}"  >
                        </div>
                    </td>
                ...
                <tr>
                    ...
                    <td>
                        <select  name="platform_id"  class="form-control input-xs"><option value>所有方式</option>@foreach($aPaymentPlatform as $id =>$name) <option value="{{$id}}"@if(@strlen($aSearchFields['platform_id'])>0&&@$aSearchFields['platform_id']==$id)selected='selected' @endif>{{___('_deposit.'.$name)}}</option> @endforeach</select>
                        <input name='deposit_mode' value='{{$aSearchFields['deposit_mode']}}' type='hidden'/>
                    </td>
                    @endif
                </tr>
                <tr>
                    ...
                    <td>
                        <select  name="status"  class="form-control input-xs"><option value>所有状态</option>@foreach($validStatuses as $id =>$name) <option value="{{$id}}"@if(@strlen($aSearchFields['status'])>0&&@$aSearchFields['status']==$id)selected='selected' @endif>{{___('_deposit.'.$name)}}</option> @endforeach</select>
                    </td>
